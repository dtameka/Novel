...
define gui.about = _p("""
    Наша команда и их Discord:

    Сценарист - @.mvinnie

    Звукооператор - @williamnalls
    ...
    Большая благодарность DJ Saint Kuz за предоставленные сэмплы.

    Так же хотелось отметить, что мы опирались на примеры кода разработчиков с Itch.io

    И использовали некоторый MIT лизенцированный контент, данных людей:

    Wattson - Автоподсветка персонажей, форматирование текста.

    kyouryuukunn - Удобный просмотр сцен (Был удален, но использовался при разработке).

    tofurocks - работа с парктиклами.
""")
...
define config.save_directory = "Jamnovel-1715191280"
