...
label prologue_01: # Пробуждение
    ...
    {glitch=20}Как же... болит голова...{/glitch}
     
    {fi=[0]-[1]-[0]}Меня словно лопатой приложили...{/fi}
    ''' with dissolve
        ...
        menu:
            "Проверить карманы" if not check_ticket:
                ...
                if light_on:                  
                    ...
                    player_base '''
                    {fi=[0]-[1]-[0]}Для начала найду способ включить свет,\nчтобы хоть что-то разобрать. Листок подождет.{/fi}
                    '''
...
label prologue_02:
    ...
    while not exit:
        menu:
            ...
            "Осмотреться":
                ...
                Но лопата, что она здесь делает? Может именно ей вы получили по голове...?
                '''             
    ...
    show alice baseA with dissolve
