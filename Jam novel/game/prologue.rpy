label brief: # Бриф
    ...
    $ scenep()
    $ showp("bg radio_antenna") 
    with ImageDissolve("transitions/wet.webp", 4.0, 50)

    '''
    {fi=[0]-[2]-[0]}И мы стали отправлять сигналы в надежде быть услышанными, \nв надежде что кто-то поможет нам преодолеть\n 
    звёздные рубежи в этом огромном пространстве пустоты.{/fi}
